...
export default {
    ...
    methods: {
        ...
        prepareCreatedItemsForHistory () {
            ...
            const = {};
            const { id: providerId, name, URC } = this.$store.state.selectedProvider;

            const createdProductsForHistory = this.createdItems.map(createdItem => {
                const { id: product_name: productObj, ...itemWithoutProductObj } = createdItem;
                const { ...productObjWithoutId } = productObj;
